    ...
    */
    internal sealed class BigEndianBinaryReader
    {
        ...
        internal string ReadString(string classFile)
        {
            ...
            if (end - pos < len)
            {
                throw new ClassFileFormatException(string.Format("{0} (Truncated class file)", classFile));
            }
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        switch (c >> 4)
                        {
                            case 0:
                                if (c == 0)
                                {
                                    throw new ClassFileFormatException(string.Format("{0} (Illegal UTF8 string in constant pool)", classFile));
                                }
                                break;
                            case 1:
                            case 2:
